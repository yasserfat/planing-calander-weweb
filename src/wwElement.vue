...
export default {
    ...
    setup(props, { emit }) {
        ...
        // Computed key to force remount when defaultView or slot settings change
        const calendarKey = computed(() => {
            return `calendar-${props.content?.defaultView || 'dayGridMonth'}-${props.content?.slotHeight || 'default'}-${props.content?.slotDuration || 'default'}`;
        });
        ...
        const calendarStyles = computed(() => ({
            ...
            '--fc-slot-height': props.content?.slotHeight || '24px', // Bind CSS variable for slot height
        }));
        ...
        const calendarOptions = computed(() => {
            ...
            return {
                ...
                slotDuration: props.content?.slotDuration || '01:00:00', // Default to 1 hour slots
                ...
                datesSet: info => {
                    ...
                    // UPDATE: Set the new variables
                    setStartDate(info.view.activeStart?.toISOString() || null);
                    setEndDate(info.view.activeEnd?.toISOString() || null);
